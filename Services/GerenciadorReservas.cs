...
public class GerenciadorReservas : IGerenciadorReservas
{
                                 ...
                                 HotelDirector director, DateTime entrada, DateTime saida)
    {
        ...
        AnexarObservadores(reserva);
        // A reserva já inicia no estado Confirmada
    ...
    public bool RealizarCheckIn(string idReserva)
    {
        ...
        {
            _notificacao.Informar($"⚠️ {ex.Message}");
            return false;
        }
    }
        ...
        string metodoPagamento = "Pix", string? numeroCartao = null, string? cvv = null)
    {
        ...
        reserva.DefinirEstrategiaPagamento(EstrategiaPagamentoFactory.Criar(metodoPagamento, numeroCartao, cvv));
        var resultadoPagamento = reserva.ProcessarPagamento(valorTotal);

        if (resultadoPagamento?.Sucesso != true)
        {
        ...
        AnexarObservadores(reserva);
        // Reserva já inicia como Confirmada via IEstadoReserva default
        _repositorio.Adicionar(reserva);

        return reserva;
    }

    private void AnexarObservadores(Reserva reserva)
    {
        foreach (var observador in _observadores)
            reserva.Anexar(observador);
    }
}